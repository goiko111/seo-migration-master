...
interface LangContent {
  ...
  wineTypes: WineTypeI18n[];
  decides: string[]; avoids: string[]; impact: string[];
  inputTitle: string; wineTypeLabel: string; suggestedMult: string;
  ...
  insightText: (glasses: number, glassPrice: string, breakEven: number) => string;
...
const i18n: Record<string, LangContent> = {
  es: {
    ...
    subtitle: "Descubre cuánto deberías cobrar por cada botella y copa para mantener un margen rentable.",
    demoTitle: "Demo del motor de pricing y rentabilidad de Winerim Core",
    demoDesc: "Esta herramienta muestra una versión simplificada de cómo Winerim ayuda a definir precios con más criterio según coste, tipo de vino, margen objetivo y contexto del restaurante.",
    wineTypes: [
    ...
    inputTitle: "Datos de tu vino", wineTypeLabel: "Tipo de vino", suggestedMult: "Multiplicador sugerido",
    ...
    pvpBottle: "PVP recomendado por botella", pvpGlass: "PVP recomendado por copa",
    ...
    insightText: (glasses, glassPrice, breakEven) => `Con ${glasses} copas a ${glassPrice} € cada una, las primeras ${breakEven} copas cubren el coste. El resto es beneficio neto.`,
    ...
    glassRuleDesc: "El precio de una copa de vino debería ser suficiente para que las 2-3 primeras copas vendidas cubran el coste de la botella completa. Así, aunque se pierdan 1-2 copas por merma, la operación sigue siendo rentable. Es la forma más segura de garantizar margen en la venta por copa.",
    errorsTitle: "Errores comunes de pricing",
    errors: ["Usar el mismo multiplicador para todos los vinos", "No tener escalera de precios progresiva", "Dejar huecos entre rangos de precio", "Fijar precios sin analizar la competencia"],
  ...
  en: {
    ...
    subtitle: "Discover how much you should charge per bottle and glass to maintain a profitable margin.",
    demoTitle: "Demo of Winerim Core's pricing & profitability engine",
    demoDesc: "This tool shows a simplified version of how Winerim helps set prices with better criteria based on cost, wine type, target margin and restaurant context.",
    wineTypes: [
      { id: "entry", label: "Entry-level", sublabel: "< €8", defaultMult: 3.5, icon: "🍷" },
      { id: "mid", label: "Mid-range", sublabel: "€8–20", defaultMult: 2.8, icon: "🍷" },
      { id: "premium", label: "Premium", sublabel: "€20–50", defaultMult: 2.2, icon: "🥂" },
      { id: "icon", label: "Iconic", sublabel: "> €50", defaultMult: 1.8, icon: "🏆" },
    ],
    decides: [
      "Which multiplier to apply based on range and wine type",
    ...
    avoids: [
      "Applying the same multiplier across the entire list without criteria",
      "Leaving price gaps that block sales",
      "Setting prices that don't cover the cost per glass",
    ],
    impact: [
    ...
    inputTitle: "Your wine data", wineTypeLabel: "Wine type", suggestedMult: "Suggested multiplier",
    ...
    insightText: (glasses, glassPrice, breakEven) => `With ${glasses} glasses at €${glassPrice} each, the first ${breakEven} glasses cover the cost. The rest is net profit.`,
    ...
    errors: ["Using the same multiplier for all wines", "No progressive price ladder", "Leaving gaps between price ranges", "Setting prices without analysing competition"],
  ...
  it: {
    ...
    subtitle: "Scopri quanto dovresti far pagare per ogni bottiglia e calice per mantenere un margine redditizio.",
    demoTitle: "Demo del motore di pricing e redditività di Winerim Core",
    demoDesc: "Questo strumento mostra una versione semplificata di come Winerim aiuta a definire i prezzi con maggiori criteri in base a costo, tipo di vino, margine obiettivo e contesto del ristorante.",
    wineTypes: [
      { id: "entry", label: "Ingresso", sublabel: "< 8 €", defaultMult: 3.5, icon: "🍷" },
      { id: "mid", label: "Gamma media", sublabel: "8–20 €", defaultMult: 2.8, icon: "🍷" },
      { id: "premium", label: "Premium", sublabel: "20–50 €", defaultMult: 2.2, icon: "🥂" },
      { id: "icon", label: "Iconico", sublabel: "> 50 €", defaultMult: 1.8, icon: "🏆" },
    ],
    decides: [
      "Quale moltiplicatore applicare in base alla gamma e al tipo di referenza",
    ...
    avoids: [
      "Applicare lo stesso moltiplicatore a tutta la carta senza criterio",
      "Lasciare vuoti di prezzo che bloccano la vendita",
      "Fissare prezzi che non coprono il costo per calice",
    ],
    impact: [
    ...
    inputTitle: "Dati del tuo vino", wineTypeLabel: "Tipo di vino", suggestedMult: "Moltiplicatore suggerito",
    ...
    pvpBottle: "Prezzo consigliato per bottiglia", pvpGlass: "Prezzo consigliato per calice",
    ...
    insightText: (glasses, glassPrice, breakEven) => `Con ${glasses} calici a ${glassPrice} € ciascuno, i primi ${breakEven} calici coprono il costo. Il resto è profitto netto.`,
    ...
    glassRuleDesc: "Il prezzo di un calice di vino dovrebbe essere sufficiente affinché i primi 2-3 calici venduti coprano il costo dell'intera bottiglia. Così, anche perdendo 1-2 calici per scarti, l'operazione resta redditizia.",
    errorsTitle: "Errori comuni di pricing",
    errors: ["Usare lo stesso moltiplicatore per tutti i vini", "Non avere una scala di prezzi progressiva", "Lasciare vuoti tra le fasce di prezzo", "Fissare i prezzi senza analizzare la concorrenza"],
    ctaBadge: "Winerim Core", ctaTitle: "Questo calcolatore è solo l'", ctaTitleHighlight: "inizio",
    ctaDesc: "Winerim Core analizza pricing, margini, rotazione e l'architettura completa della tua carta — con 26 moduli specializzati che lavorano con i tuoi dati reali, non con formule generiche.",
    ...
    internalLinks: [
      ...
      { to: "/blog/como-disenar-carta-vinos-rentable", label: "Come creare una carta redditizia", type: "guide" },
      { to: "/wine-pricing-tool", label: "Strumento di pricing", type: "tool" },
      { to: "/recursos/plantilla-wine-mapping-restaurante", label: "Template wine mapping", type: "resource" },
  ...
  fr: {
    ...
    subtitle: "Découvrez combien vous devriez facturer par bouteille et par verre pour maintenir une marge rentable.",
    demoTitle: "Démo du moteur de pricing et de rentabilité de Winerim Core",
    demoDesc: "Cet outil montre une version simplifiée de la façon dont Winerim aide à définir les prix avec plus de critères selon le coût, le type de vin, la marge cible et le contexte du restaurant.",
    wineTypes: [
      { id: "entry", label: "Entrée de gamme", sublabel: "< 8 €", defaultMult: 3.5, icon: "🍷" },
      { id: "mid", label: "Milieu de gamme", sublabel: "8–20 €", defaultMult: 2.8, icon: "🍷" },
      { id: "premium", label: "Premium", sublabel: "20–50 €", defaultMult: 2.2, icon: "🥂" },
      { id: "icon", label: "Iconique", sublabel: "> 50 €", defaultMult: 1.8, icon: "🏆" },
    ],
    decides: [
      "Quel multiplicateur appliquer selon la gamme et le type de référence",
    ...
    avoids: [
      "Appliquer le même multiplicateur à toute la carte sans critère",
      "Laisser des trous de prix qui bloquent la vente",
      "Fixer des prix qui ne couvrent pas le coût par verre",
    ],
    impact: [
    ...
    inputTitle: "Données de votre vin", wineTypeLabel: "Type de vin", suggestedMult: "Multiplicateur suggéré",
    ...
    pvpBottle: "Prix de vente recommandé par bouteille", pvpGlass: "Prix de vente recommandé par verre",
    ...
    insightText: (glasses, glassPrice, breakEven) => `Avec ${glasses} verres à ${glassPrice} € chacun, les ${breakEven} premiers verres couvrent le coût. Le reste est du bénéfice net.`,
    ...
    glassRuleDesc: "Le prix d'un verre de vin devrait être suffisant pour que les 2-3 premiers verres vendus couvrent le coût de la bouteille entière. Ainsi, même en perdant 1-2 verres par perte, l'opération reste rentable.",
    errorsTitle: "Erreurs courantes de pricing",
    errors: ["Utiliser le même multiplicateur pour tous les vins", "Pas d'échelle de prix progressive", "Laisser des trous entre les gammes de prix", "Fixer les prix sans analyser la concurrence"],
    ctaBadge: "Winerim Core", ctaTitle: "Ce calculateur n'est que le", ctaTitleHighlight: "début",
    ctaDesc: "Winerim Core analyse le pricing, les marges, la rotation et l'architecture complète de votre carte — avec 26 modules spécialisés qui travaillent avec vos données réelles, pas des formules génériques.",
    ...
    internalLinks: [
      ...
      { to: "/wine-pricing-tool", label: "Outil de pricing", type: "tool" },
      { to: "/recursos/plantilla-wine-mapping-restaurante", label: "Template wine mapping", type: "resource" },
...
const CalculadoraMargen = () => {
  ...
  const results = useMemo(() => {
    ...
    return { bottlePrice, glassPrice, grossMargin, marginPercent, costPerGlass, breakEvenGlasses, currentMargin, currentMarginPct, priceDiff };
  ...
  useEffect(() => {
    ...
    schema.textContent = JSON.stringify({
      ...
      "@type": "FAQPage",
      mainEntity: [
        { "@type": "Question", name: "¿Cómo se calcula el precio de venta del vino en un restaurante?", acceptedAnswer: { "@type": "Answer", text: "Se multiplica el precio de compra por un factor (normalmente entre 2 y 4)." } },
        { "@type": "Question", name: "¿Cuál es el margen habitual del vino en hostelería?", acceptedAnswer: { "@type": "Answer", text: "El margen bruto del vino en restaurantes suele estar entre el 60% y el 75%." } },
      ],
    });
          ...
          <div className="grid lg:grid-cols-2 gap-8">
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-[10px] text-muted-foreground mt-1.5">
                    {t.suggestedMult}: ×{t.wineTypes.find((w) => w.id === wineType)?.defaultMult}
                  </p>
                </div>
                ...
                <div>
                  ...
                  <input type="range" min={1.5} max={5} step={0.1} value={multiplier}
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8 }}
              ...
              <div className="space-y-4">
                ...
                <ResultCard label={t.costPerGlass} value={`${results.costPerGlass.toFixed(2)} €`} />
                <ResultCard label={t.glassesToCover} value={`${results.breakEvenGlasses} ${t.glassesUnit}`} sub={t.ofTotal.replace("{n}", String(glassesPerBottle))} />
                ...
                {results.priceDiff !== null && currentPrice && (
                  <div className={`rounded-xl border p-4 ${results.priceDiff > 0 ? "bg-emerald-500/5 border-emerald-500/20" : results.priceDiff < 0 ? "bg-amber-500/5 border-amber-500/20" : "bg-secondary/50 border-border"}`}>
                    <p className="text-xs text-muted-foreground mb-1">{t.compareLabel}</p>
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm font-medium">
                          {t.yourPrice}: <span className="font-bold">{currentPrice.toFixed(2)} €</span>
                          <span className="text-muted-foreground mx-1.5">→</span>
                          {t.recommended}: <span className="font-bold text-wine">{results.bottlePrice.toFixed(2)} €</span>
                        </p>
                        <p className="text-xs text-muted-foreground mt-0.5">
                          {t.yourMargin}: {results.currentMargin?.toFixed(2)} € ({results.currentMarginPct?.toFixed(0)}%)
                        </p>
                      </div>
                      <div className="text-right">
                        <p className={`font-heading text-lg font-bold ${results.priceDiff! > 0 ? "text-emerald-500" : results.priceDiff! < 0 ? "text-amber-500" : "text-foreground"}`}>
