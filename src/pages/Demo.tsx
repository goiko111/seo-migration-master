          ...
          <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-16 items-center">
            <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }}>
              ...
              <h1 className="font-heading text-4xl md:text-5xl font-bold mb-6 leading-tight">
                {get("hero", "title", "Descubre cómo Winerim")}{" "}
                <span className="text-gradient-wine">{get("hero", "title_highlight", "transforma tu carta de vinos")}</span>
              </h1>
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.15 }}
              ...
              <form className="space-y-5" onSubmit={handleSubmit}>
                ...
                <p className="text-xs text-muted-foreground text-center">
                  {get("form", "disclaimer", "Al enviar aceptas nuestra política de privacidad.")}
                </p>
              </form>
            </motion.div>
          </div>
