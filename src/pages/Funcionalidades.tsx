...
import featureCloudRimInboxImg from "@/assets/feature-cloudrim-inbox-current.webp";
import featureCloudRimFlowImg from "@/assets/feature-cloudrim-document-flow.webp";
import featureMarginsOverviewImg from "@/assets/feature-margins-overview.webp";
import featureMarginsWineDetailImg from "@/assets/feature-margins-wine-detail.webp";
import featureSupplyDistributorsImg from "@/assets/feature-supply-distributors.webp";
import featureSupplyComparisonImg from "@/assets/feature-supply-tariff-comparison.webp";
import featureSaviaPromptImg from "@/assets/feature-savia-prompt.webp";
import featureSaviaAnswerImg from "@/assets/feature-savia-margin-answer.webp";
...
const SHOTS = {
  ...
  cloudRim: [
    ...
    { img: featureCloudRimFlowImg, label: "Trazabilidad" },
  ],
  savia: [
    { img: featureSaviaPromptImg, label: "Consulta" },
    { img: featureSaviaAnswerImg, label: "Respuesta" },
  ],
  pricing: [
    { img: featureMarginsOverviewImg, label: "Visión de bodega" },
    { img: featureMarginsWineDetailImg, label: "Margen por referencia" },
  ],
  supply: [
    { img: featureSupplyDistributorsImg, label: "Distribuidores" },
    { img: featureSupplyComparisonImg, label: "Comparador" },
  ],
  analytics: [
...
const content: I18nMap<PageContent> = {
  es: {
    ...
    features: [
      ...
      },
      {
        ...
        desc: "Control de inventario conectado con ventas y carta. La frecuencia de actualización depende del conector y del flujo validado; Winerim detecta stock muerto, rotación lenta y capital inmovilizado.",
      ...
      },
      {
        ...
        screenshots: SHOTS.pricing.map((s, j) => ({ ...s, label: j === 0 ? "Visión de bodega" : "Margen por referencia", alt: j === 0 ? "Dashboard general de márgenes y salud de la bodega" : "Análisis de margen por referencia" })),
      },
      {
      ...
      {
        icon: ShoppingCart, title: "Winerim Supply",
        desc: "Conecta distribuidores, tarifas, costes reales, stock, rotación y pedidos para convertir la reposición en una decisión trazable, no en una repetición por costumbre.",
        benefits: ["Compara costes y condiciones entre distribuidores", "Detecta sobreprecios y desviaciones de tarifa", "Cruza demanda, cobertura y margen antes de reponer", "Prepara pedidos y alternativas para revisión humana"],
        linkLabel: "Explorar Winerim Supply", linkHref: "/producto/winerim-supply",
        screenshots: SHOTS.supply.map((s, j) => ({ ...s, label: j === 0 ? "Distribuidores" : "Comparador", alt: j === 0 ? "Inteligencia de distribuidores" : "Comparador de tarifas de distribuidores" })),
      },
      {
        icon: Cloud, title: "CloudRIM",
        desc: "La nube operativa donde el restaurante deja cartas, albaranes, facturas, ventas, stock y tarifas. Winerim clasifica cada documento, distingue factura vs albarán, extrae la información relevante y la envía al flujo correcto.",
        benefits: ["Recibe documentos por portal, email, carpeta compartida, FTP/SFTP, API o proveedor", "Clasifica cartas, ventas, stock, albaranes, facturas y tarifas", "Prepara conciliación albarán-factura antes de tocar costes o márgenes", "Reduce introducción manual y duplicidad operativa", "Deja trazabilidad sobre lo recibido, procesado y pendiente de revisión"],
        linkLabel: "Conocer CloudRIM", linkHref: "/producto/cloudrim",
        screenshots: SHOTS.cloudRim.map((s, j) => ({ ...s, label: j === 0 ? "Bandeja" : "Trazabilidad", alt: j === 0 ? "Bandeja CloudRIM de documentos" : "Trazabilidad de un documento en CloudRIM" })),
      },
      {
        icon: MessageSquare, title: "SAVia",
        desc: "El agente IA de Winerim. Pregunta por ventas, margen, stock, costes, facturas, albaranes o RIMs y recibe respuestas accionables para decidir qué revisar, comprar, impulsar o preparar.",
        benefits: ["Consulta carta, ventas, stock, costes, márgenes, albaranes, facturas y RIMs", "Explica oportunidades y riesgos en lenguaje operativo", "Prepara decisiones y previews sin ejecutar acciones críticas sin aprobación humana", "Ayuda a dirección, sumiller y sala a decidir con contexto"],
        linkLabel: "Ver SAVia", linkHref: "/producto/savia",
        screenshots: SHOTS.savia.map((s, j) => ({ ...s, label: j === 0 ? "Consulta" : "Respuesta", alt: j === 0 ? "Consulta en el panel conversacional de SAVia" : "Respuesta de SAVia sobre rendimiento y margen" })),
      },
      {
    ...
    comparisonRows: [
      ...
      { feature: "Control de stock conectado", winerim: "✅", cartaQR: "❌", digitalBasica: "❌", tradicional: "Manual" },
    ...
    faqs: [
      { q: "¿Winerim funciona con mi TPV actual?", a: "Ágora dispone de un flujo verificado. Para REVO XEF y otros TPV evaluamos accesos, datos disponibles, mapeo y frecuencia antes de confirmar alcance, plazo y coste." },
      ...
      { q: "¿Qué pasa si un vino se agota?", a: "Con el stock y la integración correctamente configurados, Winerim actualiza la disponibilidad según la frecuencia validada. Si el conector no permite esa automatización, el equipo mantiene el control manual desde Winerim." },
  ...
  en: {
    ...
    features: [
      {
        ...
        screenshots: SHOTS.carta.map((s, j) => ({ ...s, label: j === 0 ? "Wine list" : "Comparison", alt: j === 0 ? "Interactive wine list on tablet" : "Wine comparator" })),
      },
      {
        ...
        screenshots: SHOTS.ai.map((s, j) => ({ ...s, label: j === 0 ? "Pairing" : "AI settings", alt: j === 0 ? "Automatic wine pairing" : "AI pairing configuration" })),
      },
      {
        icon: Package, title: "Stock & inventory management",
        desc: "Inventory control connected to sales and the wine list. Update frequency depends on the validated connector and workflow; Winerim detects dead stock, slow rotation and tied-up capital.",
        ...
        screenshots: SHOTS.stock.map((s, j) => ({ ...s, label: j === 0 ? "Stock" : "Obsolescence", alt: j === 0 ? "Wine stock control" : "Obsolescence detection" })),
      },
      {
        ...
        screenshots: SHOTS.cellarMap.map((s, j) => ({ ...s, label: j === 0 ? "Wine Cellar" : "Maps", alt: j === 0 ? "Wine Cellar map" : "Stock connected to cellar map" })),
      },
      {
        icon: LockKeyhole, title: "Wine Lockers",
        ...
        screenshots: SHOTS.lockers.map((s, j) => ({ ...s, label: j === 0 ? "Wine Lockers" : "Guest", alt: j === 0 ? "Wine Lockers summary" : "Wine Lockers area and private summary" })),
      },
      {
        ...
        screenshots: SHOTS.pricing.map((s, j) => ({ ...s, label: j === 0 ? "Cellar overview" : "Margin by SKU", alt: j === 0 ? "General margin and cellar health dashboard" : "Margin analysis by wine SKU" })),
      },
      {
        ...
        screenshots: SHOTS.analytics.map((s, j) => ({ ...s, label: j === 0 ? "Insights" : "Performance", alt: j === 0 ? "Insights and KPIs dashboard" : "Overall wine list performance" })),
      },
      {
        ...
        screenshots: SHOTS.iaApplied.map((s, j) => ({ ...s, label: j === 0 ? "AI wine sheet" : "RIM engine", alt: j === 0 ? "Detailed wine sheet" : "RIM recommendation engine" })),
      },
      {
        ...
        screenshots: SHOTS.dynamic.map((s, j) => ({ ...s, label: j === 0 ? "Recommendations" : "Automation", alt: j === 0 ? "Dynamic recommendations" : "Automations" })),
      },
      {
        icon: ShoppingCart, title: "Winerim Supply",
        desc: "Connects suppliers, tariffs, true costs, stock, rotation and orders so replenishment becomes a traceable decision rather than a habitual repeat purchase.",
        benefits: ["Compares costs and terms across suppliers", "Flags overpricing and tariff deviations", "Combines demand, cover and margin before reordering", "Prepares orders and alternatives for human review"],
        linkLabel: "Explore Winerim Supply", linkHref: "/producto/winerim-supply",
        screenshots: SHOTS.supply.map((s, j) => ({ ...s, label: j === 0 ? "Suppliers" : "Comparison", alt: j === 0 ? "Supplier intelligence" : "Supplier tariff comparison" })),
      },
      {
        icon: Cloud, title: "CloudRIM",
        desc: "The operating cloud where the restaurant drops wine lists, delivery notes, invoices, sales, stock and distributor tariffs. Winerim classifies each document, distinguishes invoice vs delivery note, extracts the relevant information and routes it to the right workflow.",
        benefits: ["Collects files via portal, email, shared folder, FTP/SFTP, API or provider", "Classifies lists, sales, stock, delivery notes, invoices and tariffs", "Prepares delivery-note-to-invoice reconciliation before touching costs or margins", "Reduces manual entry and duplicated work", "Tracks what arrived, what was processed and what needs review"],
        linkLabel: "Meet CloudRIM", linkHref: "/producto/cloudrim",
        screenshots: SHOTS.cloudRim.map((s, j) => ({ ...s, label: j === 0 ? "Inbox" : "Traceability", alt: j === 0 ? "CloudRIM document inbox" : "Document traceability in CloudRIM" })),
      },
      {
        icon: MessageSquare, title: "SAVia",
        desc: "Winerim's AI agent. Ask about sales, margin, stock, costs, invoices, delivery notes or RIMs and get actionable answers to decide what to review, buy, push or prepare.",
        benefits: ["Queries wine list, sales, stock, costs, margins, delivery notes, invoices and RIMs", "Explains opportunities and risks in operating language", "Prepares decisions and previews without executing critical actions without human approval", "Helps owners, sommeliers and floor teams decide with context"],
        linkLabel: "See SAVia", linkHref: "/producto/savia",
        screenshots: SHOTS.savia.map((s, j) => ({ ...s, label: j === 0 ? "Question" : "Answer", alt: j === 0 ? "Question in the SAVia conversational panel" : "SAVia answer about performance and margin" })),
      },
      {
        ...
        screenshots: SHOTS.training.map((s, j) => ({ ...s, label: j === 0 ? "Wine sheet" : "Pairing", alt: j === 0 ? "Tasting sheet for the team" : "Pairing guide for service" })),
      },
    ],
    comparisonRows: [
      ...
      { feature: "Connected stock control", winerim: "✅", cartaQR: "❌", digitalBasica: "❌", tradicional: "Manual" },
    ...
    faqs: [
      { q: "Does Winerim work with my current POS?", a: "Agora has a verified workflow. For REVO XEF and other POS systems, we assess access, available data, mapping and frequency before confirming scope, timing and cost." },
      ...
      { q: "What happens when a wine runs out?", a: "With stock and the integration correctly configured, Winerim updates availability at the validated frequency. If the connector does not support that automation, the team retains manual control in Winerim." },
  ...
  it: {
    ...
    features: [
      {
        ...
        screenshots: SHOTS.carta.map((s, j) => ({ ...s, label: j === 0 ? "Carta" : "Confronto", alt: j === 0 ? "Carta interattiva dei vini su tablet" : "Comparatore di vini" })),
      },
      {
        ...
        screenshots: SHOTS.ai.map((s, j) => ({ ...s, label: j === 0 ? "Abbinamento" : "Config. IA", alt: j === 0 ? "Abbinamento automatico dei vini" : "Configurazione IA e abbinamento" })),
      },
      {
        icon: Package, title: "Gestione stock e inventario",
        desc: "Controllo dell'inventario collegato a vendite e carta. La frequenza dipende dal connettore e dal flusso validato; Winerim rileva stock morto, rotazione lenta e capitale immobilizzato.",
        ...
        screenshots: SHOTS.stock.map((s, j) => ({ ...s, label: j === 0 ? "Stock" : "Obsolescenza", alt: j === 0 ? "Controllo stock vini" : "Rilevamento obsolescenza" })),
      },
      {
        ...
        screenshots: SHOTS.cellarMap.map((s, j) => ({ ...s, label: j === 0 ? "Wine Cellar" : "Mappe", alt: j === 0 ? "Mappa Wine Cellar" : "Stock collegato alla mappa cantina" })),
      },
      {
        icon: LockKeyhole, title: "Wine Lockers",
        ...
        screenshots: SHOTS.lockers.map((s, j) => ({ ...s, label: j === 0 ? "Wine Lockers" : "Cliente", alt: j === 0 ? "Riepilogo Wine Lockers" : "Area Wine Lockers e riepilogo privato" })),
      },
      {
        ...
        screenshots: SHOTS.pricing.map((s, j) => ({ ...s, label: j === 0 ? "Visione cantina" : "Margine per referenza", alt: j === 0 ? "Dashboard generale di margini e salute della cantina" : "Analisi del margine per referenza" })),
      },
      {
        ...
        screenshots: SHOTS.analytics.map((s, j) => ({ ...s, label: j === 0 ? "Insight" : "Performance", alt: j === 0 ? "Dashboard insights e KPI" : "Rendimento globale carta" })),
      },
      {
        ...
        screenshots: SHOTS.iaApplied.map((s, j) => ({ ...s, label: j === 0 ? "Scheda IA" : "Motore RIM", alt: j === 0 ? "Scheda vino dettagliata" : "Motore di raccomandazione RIM" })),
      },
      {
        ...
        screenshots: SHOTS.dynamic.map((s, j) => ({ ...s, label: j === 0 ? "Raccomandati" : "Automazione", alt: j === 0 ? "Raccomandazioni dinamiche" : "Automatizzazioni" })),
      },
      {
        icon: ShoppingCart, title: "Winerim Supply",
        desc: "Collega distributori, listini, costi reali, stock, rotazione e ordini per rendere il riordino una decisione tracciabile.",
        benefits: ["Confronta costi e condizioni tra distributori", "Segnala sovrapprezzi e scostamenti di listino", "Incrocia domanda, copertura e margine prima del riordino", "Prepara ordini e alternative per la revisione umana"],
        linkLabel: "Esplora Winerim Supply", linkHref: "/producto/winerim-supply",
        screenshots: SHOTS.supply.map((s, j) => ({ ...s, label: j === 0 ? "Distributori" : "Confronto", alt: j === 0 ? "Intelligenza distributori" : "Confronto listini distributori" })),
      },
      {
        icon: Cloud, title: "CloudRIM",
        ...
        screenshots: SHOTS.cloudRim.map((s, j) => ({ ...s, label: j === 0 ? "Bacheca" : "Tracciabilità", alt: j === 0 ? "Bacheca documenti CloudRIM" : "Tracciabilità di un documento in CloudRIM" })),
      },
      {
        icon: MessageSquare, title: "SAVia",
        ...
        screenshots: SHOTS.savia.map((s, j) => ({ ...s, label: j === 0 ? "Domanda" : "Risposta", alt: j === 0 ? "Domanda nel pannello conversazionale SAVia" : "Risposta SAVia su rendimento e margine" })),
      },
      {
        ...
        screenshots: SHOTS.training.map((s, j) => ({ ...s, label: j === 0 ? "Scheda" : "Abbinamento", alt: j === 0 ? "Scheda di degustazione per il team" : "Guida agli abbinamenti per la sala" })),
      },
    ],
    comparisonRows: [
      ...
      { feature: "Controllo stock collegato", winerim: "✅", cartaQR: "❌", digitalBasica: "❌", tradicional: "Manuale" },
    ...
    faqs: [
      { q: "Winerim funziona con il mio POS attuale?", a: "Agora dispone di un flusso verificato. Per REVO XEF e altri POS valutiamo accessi, dati, mappatura e frequenza prima di confermare ambito, tempi e costi." },
      ...
      { q: "Cosa succede quando un vino si esaurisce?", a: "Con stock e integrazione configurati correttamente, Winerim aggiorna la disponibilità alla frequenza validata. Se il connettore non lo consente, il team mantiene il controllo manuale in Winerim." },
  ...
  fr: {
    ...
    features: [
      {
        ...
        screenshots: SHOTS.carta.map((s, j) => ({ ...s, label: j === 0 ? "Carte" : "Comparateur", alt: j === 0 ? "Carte interactive des vins sur tablette" : "Comparateur de vins" })),
      },
      {
        ...
        screenshots: SHOTS.ai.map((s, j) => ({ ...s, label: j === 0 ? "Accord" : "Configuration IA", alt: j === 0 ? "Accord automatique des vins" : "Configuration IA et accord" })),
      },
      {
        icon: Package, title: "Gestion de stock et inventaire",
        desc: "Contrôle d'inventaire connecté aux ventes et à la carte. La fréquence dépend du connecteur et du flux validé ; Winerim détecte stock mort, rotation lente et capital immobilisé.",
        ...
        screenshots: SHOTS.stock.map((s, j) => ({ ...s, label: j === 0 ? "Stock" : "Obsolescence", alt: j === 0 ? "Contrôle du stock de vins" : "Détection d'obsolescence" })),
      },
      {
        ...
        screenshots: SHOTS.cellarMap.map((s, j) => ({ ...s, label: j === 0 ? "Wine Cellar" : "Plans", alt: j === 0 ? "Plan Wine Cellar" : "Stock connecte au plan de cave" })),
      },
      {
        icon: LockKeyhole, title: "Wine Lockers",
        ...
        screenshots: SHOTS.lockers.map((s, j) => ({ ...s, label: j === 0 ? "Wine Lockers" : "Client", alt: j === 0 ? "Resume Wine Lockers" : "Espace Wine Lockers et resume prive" })),
      },
      {
        ...
        screenshots: SHOTS.pricing.map((s, j) => ({ ...s, label: j === 0 ? "Vue de la cave" : "Marge par référence", alt: j === 0 ? "Tableau général des marges et de la santé de la cave" : "Analyse de marge par référence" })),
      },
      {
        ...
        screenshots: SHOTS.analytics.map((s, j) => ({ ...s, label: j === 0 ? "Insights" : "Performance", alt: j === 0 ? "Dashboard insights et KPIs" : "Performance globale de la carte" })),
      },
      {
        ...
        screenshots: SHOTS.iaApplied.map((s, j) => ({ ...s, label: j === 0 ? "Fiche IA" : "Moteur RIM", alt: j === 0 ? "Fiche de vin détaillée" : "Moteur de recommandation RIM" })),
      },
      {
        ...
        screenshots: SHOTS.dynamic.map((s, j) => ({ ...s, label: j === 0 ? "Recommandations" : "Automatisation", alt: j === 0 ? "Recommandations dynamiques" : "Automatisations" })),
      },
      {
        icon: ShoppingCart, title: "Winerim Supply",
        desc: "Relie distributeurs, tarifs, coûts réels, stock, rotation et commandes pour faire du réassort une décision traçable.",
        benefits: ["Compare coûts et conditions entre distributeurs", "Signale surcoûts et écarts tarifaires", "Croise demande, couverture et marge avant le réassort", "Prépare commandes et alternatives pour validation humaine"],
        linkLabel: "Découvrir Winerim Supply", linkHref: "/producto/winerim-supply",
        screenshots: SHOTS.supply.map((s, j) => ({ ...s, label: j === 0 ? "Distributeurs" : "Comparaison", alt: j === 0 ? "Intelligence distributeurs" : "Comparaison des tarifs distributeurs" })),
      },
      {
        icon: Cloud, title: "CloudRIM",
        ...
        screenshots: SHOTS.cloudRim.map((s, j) => ({ ...s, label: j === 0 ? "Boîte de réception" : "Traçabilité", alt: j === 0 ? "Boîte de documents CloudRIM" : "Traçabilité d'un document dans CloudRIM" })),
      },
      {
        icon: MessageSquare, title: "SAVia",
        ...
        screenshots: SHOTS.savia.map((s, j) => ({ ...s, label: j === 0 ? "Question" : "Réponse", alt: j === 0 ? "Question dans le panneau conversationnel SAVia" : "Réponse SAVia sur la performance et la marge" })),
      },
      {
        ...
        screenshots: SHOTS.training.map((s, j) => ({ ...s, label: j === 0 ? "Fiche" : "Accord", alt: j === 0 ? "Fiche de degustation pour l'equipe" : "Guide d'accords pour la salle" })),
      },
    ],
    comparisonRows: [
      ...
      { feature: "Contrôle de stock connecté", winerim: "✅", cartaQR: "❌", digitalBasica: "❌", tradicional: "Manuel" },
    ...
    faqs: [
      { q: "Winerim fonctionne-t-il avec ma caisse actuelle ?", a: "Agora dispose d'un flux vérifié. Pour REVO XEF et les autres caisses, nous évaluons accès, données, mapping et fréquence avant de confirmer le périmètre, le délai et le coût." },
      ...
      { q: "Que se passe-t-il quand un vin est épuisé ?", a: "Avec le stock et l'intégration correctement configurés, Winerim met à jour la disponibilité selon la fréquence validée. Sinon, l'équipe conserve le contrôle manuel dans Winerim." },
  ...
  de: {
    ...
    features: [
      {
        ...
        screenshots: SHOTS.carta.map((s, j) => ({ ...s, label: j === 0 ? "Weinkarte" : "Vergleich", alt: j === 0 ? "Interaktive Weinkarte auf Tablet" : "Weinvergleich" })),
      },
      {
        ...
        screenshots: SHOTS.ai.map((s, j) => ({ ...s, label: j === 0 ? "Weinbegleitung" : "KI-Einstellungen", alt: j === 0 ? "Automatische Weinbegleitung" : "KI-Konfiguration" })),
      },
      {
        icon: Package, title: "Bestands- und Lagerverwaltung",
        desc: "Mit Verkäufen und Weinkarte verbundene Bestandskontrolle. Die Frequenz hängt vom validierten Connector und Ablauf ab; Winerim erkennt Totbestand, langsame Rotation und gebundenes Kapital.",
        ...
        screenshots: SHOTS.stock.map((s, j) => ({ ...s, label: j === 0 ? "Bestand" : "Obsoleszenz", alt: j === 0 ? "Weinbestandskontrolle" : "Obsoleszenz-Erkennung" })),
      },
      {
        ...
        screenshots: SHOTS.cellarMap.map((s, j) => ({ ...s, label: j === 0 ? "Wine Cellar" : "Karten", alt: j === 0 ? "Wine Cellar Kellerkarte" : "Bestand verbunden mit Kellerkarte" })),
      },
      {
        icon: LockKeyhole, title: "Wine Lockers",
        ...
        screenshots: SHOTS.lockers.map((s, j) => ({ ...s, label: j === 0 ? "Wine Lockers" : "Gast", alt: j === 0 ? "Wine Lockers Ubersicht" : "Wine Lockers Bereich und private Ubersicht" })),
      },
      {
        ...
        screenshots: SHOTS.pricing.map((s, j) => ({ ...s, label: j === 0 ? "Kellerübersicht" : "Marge pro Referenz", alt: j === 0 ? "Dashboard für Margen und Kellerzustand" : "Margenanalyse pro Weinreferenz" })),
      },
      {
        ...
        screenshots: SHOTS.analytics.map((s, j) => ({ ...s, label: j === 0 ? "Insights" : "Leistung", alt: j === 0 ? "Insights- und KPI-Dashboard" : "Gesamtleistung der Weinkarte" })),
      },
      {
        ...
        screenshots: SHOTS.iaApplied.map((s, j) => ({ ...s, label: j === 0 ? "KI-Weinprofil" : "RIM-Motor", alt: j === 0 ? "Detailliertes Weindatenblatt" : "RIM-Empfehlungsmotor" })),
      },
      {
        ...
        screenshots: SHOTS.dynamic.map((s, j) => ({ ...s, label: j === 0 ? "Empfehlungen" : "Automatisierung", alt: j === 0 ? "Dynamische Empfehlungen" : "Automatisierungen" })),
      },
      {
        icon: ShoppingCart, title: "Winerim Supply",
        desc: "Verbindet Lieferanten, Preislisten, reale Kosten, Bestand, Rotation und Bestellungen zu einer nachvollziehbaren Nachkaufentscheidung.",
        benefits: ["Vergleicht Kosten und Konditionen mehrerer Lieferanten", "Markiert Überpreise und Tarifabweichungen", "Verknüpft Nachfrage, Reichweite und Marge vor dem Nachkauf", "Bereitet Bestellungen und Alternativen zur menschlichen Freigabe vor"],
        linkLabel: "Winerim Supply entdecken", linkHref: "/producto/winerim-supply",
        screenshots: SHOTS.supply.map((s, j) => ({ ...s, label: j === 0 ? "Lieferanten" : "Vergleich", alt: j === 0 ? "Lieferantenintelligenz" : "Vergleich von Lieferantentarifen" })),
      },
      {
        icon: Cloud, title: "CloudRIM",
        ...
        screenshots: SHOTS.cloudRim.map((s, j) => ({ ...s, label: j === 0 ? "Eingang" : "Nachverfolgung", alt: j === 0 ? "CloudRIM Dokumenten-Eingang" : "Dokumentennachverfolgung in CloudRIM" })),
      },
      {
        icon: MessageSquare, title: "SAVia",
        ...
        screenshots: SHOTS.savia.map((s, j) => ({ ...s, label: j === 0 ? "Frage" : "Antwort", alt: j === 0 ? "Frage im SAVia Konversationspanel" : "SAVia Antwort zu Leistung und Marge" })),
      },
      {
        ...
        screenshots: SHOTS.training.map((s, j) => ({ ...s, label: j === 0 ? "Weinprofil" : "Begleitung", alt: j === 0 ? "Verkostungsblatt fuer das Team" : "Speisebegleitungsleitfaden fuer den Service" })),
      },
    ],
    comparisonRows: [
      ...
      { feature: "Verbundene Bestandskontrolle", winerim: "✅", cartaQR: "❌", digitalBasica: "❌", tradicional: "Manuell" },
    ...
    faqs: [
      { q: "Funktioniert Winerim mit meinem aktuellen Kassensystem?", a: "Für Agora gibt es einen verifizierten Ablauf. Bei REVO XEF und anderen Kassensystemen prüfen wir Zugänge, Daten, Mapping und Frequenz, bevor Umfang, Zeitplan und Kosten bestätigt werden." },
      ...
      { q: "Was passiert, wenn ein Wein ausverkauft ist?", a: "Bei korrekt eingerichtetem Bestand und Connector aktualisiert Winerim die Verfügbarkeit in der validierten Frequenz. Andernfalls behält das Team die manuelle Kontrolle in Winerim." },
  ...
  pt: {
    ...
    features: [
      {
        ...
        screenshots: SHOTS.carta.map((s, j) => ({ ...s, label: j === 0 ? "Carta" : "Comparador", alt: j === 0 ? "Carta interativa de vinhos em tablet" : "Comparador de vinhos" })),
      },
      {
        ...
        screenshots: SHOTS.ai.map((s, j) => ({ ...s, label: j === 0 ? "Harmonização" : "Configuração IA", alt: j === 0 ? "Harmonização automática de vinhos" : "Configuração de IA e harmonização" })),
      },
      {
        icon: Package, title: "Gestão de stock e inventário",
        desc: "Controlo de inventário ligado a vendas e carta. A frequência depende do conector e fluxo validados; a Winerim deteta stock morto, rotação lenta e capital imobilizado.",
        ...
        screenshots: SHOTS.stock.map((s, j) => ({ ...s, label: j === 0 ? "Stock" : "Obsolescência", alt: j === 0 ? "Controlo de stock de vinhos" : "Deteção de obsolescência" })),
      },
      {
        ...
        screenshots: SHOTS.cellarMap.map((s, j) => ({ ...s, label: j === 0 ? "Wine Cellar" : "Mapas", alt: j === 0 ? "Mapa Wine Cellar" : "Stock ligado ao mapa de garrafeira" })),
      },
      {
        icon: LockKeyhole, title: "Wine Lockers",
        ...
        screenshots: SHOTS.lockers.map((s, j) => ({ ...s, label: j === 0 ? "Wine Lockers" : "Cliente", alt: j === 0 ? "Resumo Wine Lockers" : "Area Wine Lockers e resumo privado" })),
      },
      {
        ...
        screenshots: SHOTS.pricing.map((s, j) => ({ ...s, label: j === 0 ? "Visão da garrafeira" : "Margem por referência", alt: j === 0 ? "Painel geral de margens e saúde da garrafeira" : "Análise de margem por referência" })),
      },
      {
        ...
        linkLabel: "Explorar Winerim Core", linkHref: "/producto/winerim-core",
        screenshots: SHOTS.analytics.map((s, j) => ({ ...s, label: j === 0 ? "Insights" : "Desempenho", alt: j === 0 ? "Dashboard de insights e KPIs" : "Rendimento global da carta" })),
      },
      {
        ...
        linkLabel: "IA para restaurantes", linkHref: "/inteligencia-artificial-restaurantes",
        screenshots: SHOTS.iaApplied.map((s, j) => ({ ...s, label: j === 0 ? "Ficha IA" : "Motor RIM", alt: j === 0 ? "Ficha de vinho detalhada" : "Motor de recomendação RIM" })),
      },
      {
        ...
        screenshots: SHOTS.dynamic.map((s, j) => ({ ...s, label: j === 0 ? "Recomendações" : "Automatização", alt: j === 0 ? "Recomendações dinâmicas" : "Automatizações" })),
      },
      {
        icon: ShoppingCart, title: "Winerim Supply",
        desc: "Liga distribuidores, tabelas, custos reais, stock, rotação e encomendas para tornar a reposição uma decisão rastreável.",
        benefits: ["Compara custos e condições entre distribuidores", "Deteta sobrepreços e desvios de tabela", "Cruza procura, cobertura e margem antes da reposição", "Prepara encomendas e alternativas para aprovação humana"],
        linkLabel: "Explorar Winerim Supply", linkHref: "/producto/winerim-supply",
        screenshots: SHOTS.supply.map((s, j) => ({ ...s, label: j === 0 ? "Distribuidores" : "Comparação", alt: j === 0 ? "Inteligência de distribuidores" : "Comparação de tabelas de distribuidores" })),
      },
      {
        icon: Cloud, title: "CloudRIM",
        ...
        screenshots: SHOTS.cloudRim.map((s, j) => ({ ...s, label: j === 0 ? "Caixa de entrada" : "Rastreabilidade", alt: j === 0 ? "Caixa de documentos CloudRIM" : "Rastreabilidade de um documento no CloudRIM" })),
      },
      {
        icon: MessageSquare, title: "SAVia",
        ...
        linkLabel: "Ver SAVia", linkHref: "/producto/savia",
        screenshots: SHOTS.savia.map((s, j) => ({ ...s, label: j === 0 ? "Pergunta" : "Resposta", alt: j === 0 ? "Pergunta no painel conversacional SAVia" : "Resposta SAVia sobre desempenho e margem" })),
      },
      {
        ...
        screenshots: SHOTS.training.map((s, j) => ({ ...s, label: j === 0 ? "Ficha" : "Harmonização", alt: j === 0 ? "Ficha de prova para a equipa" : "Guia de harmonizacao para sala" })),
      },
    ],
    comparisonRows: [
      ...
      { feature: "Controlo de stock ligado", winerim: "✅", cartaQR: "❌", digitalBasica: "❌", tradicional: "Manual" },
    ...
    faqs: [
      { q: "O Winerim funciona com o meu POS atual?", a: "A integração com Agora tem um fluxo verificado. Para REVO XEF e outros POS avaliamos acessos, dados, mapeamento e frequência antes de confirmar âmbito, prazo e custo." },
      ...
      { q: "O que acontece quando um vinho esgota?", a: "Com stock e integração corretamente configurados, a Winerim atualiza a disponibilidade na frequência validada. Caso contrário, a equipa mantém o controlo manual na Winerim." },
