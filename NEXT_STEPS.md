...
- El push a `origin` falló por falta de credenciales GitHub en este entorno, y `gh` no está instalado.
...
1. Subir `codex/wine-library-de-pt` desde un entorno autenticado o configurar credenciales GitHub para este repo.
2. Crear PR desde `codex/wine-library-de-pt`.
3. Repetir `npm run test`, `npm run build`, lint dirigido y `deno check` si hay nuevos cambios.
4. Separar en una tarea propia los 66 errores del lint global.
5. Desplegar cuando el PR esté aprobado.
6. Validar en producción sitemap, canonical, hreflang, prerender, selector de idioma y rutas localizadas.
7. Plan editorial siguiente:
