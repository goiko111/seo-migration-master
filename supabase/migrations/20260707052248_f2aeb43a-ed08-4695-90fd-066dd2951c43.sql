...
WITH rows (
  slug, title, excerpt, body, image_url, category, published_at, lang, article_group, related_links
) AS (
  VALUES
  (
...
Primeiro a pergunta operacional: a garrafa corresponde à carta e ao stock? Depois a pergunta de sala: que frase de vinte segundos explica por que faz sentido para aquela mesa? Aprender vinho é essa camada guiada. A [Biblioteca do vinho](/pt/biblioteca-vinho) continua a ser a referência para castas, regiões, estilos e glossário.

## Produtor, região e denominação

O produtor dá confiança e estilo de casa. A região traz clima, tradição e expectativa. A denominação pode acrescentar regras sobre castas, origem, rendimentos, estágio ou classificação. Mas uma denominação não garante que o vinho agrade ao cliente. Precisa de ser traduzida.

Uma frase útil não é "é uma denominação importante". É: "vem de uma zona fresca, por isso mantém acidez e funciona com o peixe", ou "esta região é conhecida por tintos estruturados, por isso faz sentido com a carne". Se o cliente não conhece o lugar, ligue origem e sensação.

## Casta, lote e sinais de estilo

Quando o rótulo indica a casta, a conversa acelera. Albariño sugere frescura e salinidade; Chardonnay pode ser tenso ou cremoso; Tempranillo varia muito entre jovem, crianza e reserva; Pinot Noir tende a leveza e finura, conforme a origem. Se a casta não aparece, região ou produtor devem carregar a explicação.

Nos lotes, listar percentagens só faz sentido se ajudar. Pode dizer: "é um lote pensado para equilíbrio: fruto, estrutura e frescura", ou "a combinação dá volume sem perder acidez". O objetivo é traduzir a informação técnica em efeito no copo.

## Colheita, álcool, estágio e formato

A colheita indica evolução, frescura e disponibilidade, mas não é nota automática de qualidade. Uma colheita antiga pode estar excelente ou cansada; um vinho jovem pode estar vibrante ou fechado. O álcool ajuda a antecipar corpo e calor. O estágio sugere textura, madeira, especiaria ou estrutura. O formato influencia conservação e serviço, sobretudo meias garrafas e magnum.

Palavras como reserva, grande reserva, sur lie, brut, seco, trocken, colheita tardia ou estágio biológico devem despoletar decisões de serviço: temperatura, copo, decantação, expectativa de doçura e harmonização. O [glossário](/pt/biblioteca-vinho/glossario) torna estas palavras úteis.

## O que não prometer

Um rótulo não prova o estado real da garrafa. Não garante um aroma preciso, uma harmonização perfeita ou o gosto do cliente. Não substitui a prova da equipa nem o histórico de vendas. Dá pistas; a sala cruza-as com prato, preço, stock e preferência.

Evite frases como "vai adorar" ou "este vinho é sempre suave". Prefira: "pela origem e estilo, deverá parecer fresco e direto", ou "se procura algo mais leve, esta opção tem menos peso do que a anterior". Essa prudência gera confiança.

## Transformar o rótulo em frase de sala

Use três partes: origem, estilo e razão da harmonização. Exemplo: "É um branco de Rías Baixas, fresco e salino; funciona com marisco porque limpa o palato sem tapar o prato". Ou: "É um tinto de Ribera com estágio, mais estruturado; faz sentido se querem acompanhar a carne com presença".

Para treinar, escolha dez garrafas reais da carta. Para cada uma escreva uma frase de quinze segundos, uma alternativa mais fresca, uma alternativa mais estruturada e um prato. Depois verifique se a carta digital mostra essas pistas. Se o rótulo informa mas a carta não traduz, o cliente depende demasiado de uma só pessoa.
...
SELECT
  rows.slug, rows.title, rows.excerpt, rows.body, rows.image_url, rows.category,
  'Winerim', true, rows.published_at::timestamptz, rows.lang, rows.article_group, rows.related_links, now()
FROM rows
